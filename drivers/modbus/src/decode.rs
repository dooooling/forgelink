...
pub fn decode_register_value(
    ...
    let value = match data_type {
        ...
        DataType::U32 => {
            RawValue::U64(u32::from_be_bytes([slice[0], slice[1], slice[2], slice[3]]) as u64)
        }
        DataType::I32 => {
            RawValue::I64(i32::from_be_bytes([slice[0], slice[1], slice[2], slice[3]]) as i64)
        }
        DataType::F32 => {
            RawValue::F64(f32::from_be_bytes([slice[0], slice[1], slice[2], slice[3]]) as f64)
        }
        DataType::U64 => RawValue::U64(u64::from_be_bytes([
            slice[0], slice[1], slice[2], slice[3], slice[4], slice[5], slice[6], slice[7],
        ])),
        DataType::I64 => RawValue::I64(i64::from_be_bytes([
            slice[0], slice[1], slice[2], slice[3], slice[4], slice[5], slice[6], slice[7],
        ])),
        DataType::F64 => RawValue::F64(f64::from_be_bytes([
            slice[0], slice[1], slice[2], slice[3], slice[4], slice[5], slice[6], slice[7],
        ])),
...
mod tests {
    ...
    fn decodes_bits_lsb_first() {
        ...
        let value =
            decode_register_value(RegisterKind::Coil, &bytes, 0, Some(&DataType::Bool)).unwrap();
        assert_eq!(value, RawValue::Bool(true));
        let value =
            decode_register_value(RegisterKind::Coil, &bytes, 1, Some(&DataType::Bool)).unwrap();
        assert_eq!(value, RawValue::Bool(false));
        let value =
            decode_register_value(RegisterKind::Coil, &bytes, 8, Some(&DataType::Bool)).unwrap();
        assert_eq!(value, RawValue::Bool(true));
    }
    ...
    fn defaults_when_type_unspecified() {
        let bytes = [0x13, 0x88];
        let value = decode_register_value(RegisterKind::HoldingRegister, &bytes, 0, None).unwrap();
        assert_eq!(value, RawValue::U64(5000));
        let bytes = [0b1010_0000];
        let value = decode_register_value(RegisterKind::Coil, &bytes, 0, None).unwrap();
        assert_eq!(value, RawValue::Bool(false));
    }

    #[test]
    fn bit_segment_rejects_register_types() {
        let err = decode_register_value(RegisterKind::Coil, &[0x00], 0, Some(&DataType::U16))
            .unwrap_err();
    ...
    fn rejects_out_of_range_offset() {
        assert!(
            decode_register_value(RegisterKind::HoldingRegister, &[0x00, 0x01], 5, None).is_err()
        );
        assert!(decode_register_value(RegisterKind::Coil, &[0x00], 8, None).is_err());
    }
