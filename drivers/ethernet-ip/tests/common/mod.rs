...
fn ensure_plugin_built() {
    ...
    BUILD_GUARD.get_or_init(|| {
        let status = std::process::Command::new("cargo")
            .args(["build", "-p", "driver-ether-ip"])
            .status()
            .expect("无法启动 cargo build");
        assert!(
            status.success(),
            "cargo build -p driver-ether-ip 失败（cdylib 产物缺失或过期）"
        );
    });
}
...
pub fn load_plugin() -> Arc<NativePlugin> {
    ...
    let manifest = DriverManifest {
        id: "ethernet-ip".to_owned(),
        name: "EtherNet/IP CIP".to_owned(),
    ...
    Arc::new(
        NativePlugin::load(&plugin_file(), manifest)
            .expect("cdylib 产物缺失，请先构建 driver-ether-ip"),
    )
}
