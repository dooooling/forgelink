//! Poll Engine × Mitsubishi MC Driver 集成测试（mock PLC → cdylib →
...
async fn polls_mc_driver_via_native_plugin() {
    // D 寄存器计数 + M 位两软元件。
    let behavior = McBehavior::new().with_d(500, 777).with_m(10, true);
    let server = McServer::start(behavior);
    let mut driver = NativeDriver::create(load_plugin(), &mc_mock::tcp_config(&server, 1000))
        .expect("create 失败");
    ...
    assert_eq!(batch.device_id, "mc-dev");
    assert_eq!(batch.results.len(), 2);
    assert_eq!(batch.results[0].value, Some(RawValue::U64(777)));
