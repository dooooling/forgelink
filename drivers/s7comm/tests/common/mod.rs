...
/// 进程统一执行一次增量 `cargo build -p driver-s7comm`。OnceLock 保证同
/// 进程只启动一次 Cargo（与 modbus 测试同模式）。
fn ensure_plugin_built() {
    ...
    BUILD_GUARD.get_or_init(|| {
        ...
        assert!(
            status.success(),
            "cargo build -p driver-s7comm 失败（cdylib 产物缺失或过期）"
        );
    });
}
...
pub fn load_plugin() -> Arc<NativePlugin> {
    ...
    Arc::new(
        NativePlugin::load(&plugin_file(), manifest)
            .expect("cdylib 产物缺失，请先构建 driver-s7comm"),
    )
}
