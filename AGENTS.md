...
- 当前已有 Cargo workspace 骨架（`crates/` 12 个占位库、`drivers/modbus` cdylib 占位、`apps/` 三个二进制占位、`profiles/` 目录）；Git/CI 状态执行任务前先核实。
...
- 工具链：Rust 1.95+，edition 2024。构建验证命令：`cargo check --workspace`（全部 16 个成员）；目前无测试和 lint 命令。
