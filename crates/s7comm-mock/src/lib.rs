    ...
    match param[0] {
        ...
        FUNCTION_READ => {
            ...
            for i in 0..count {
                ...
                // 真机实测佐证）：BIT/WORD/INT/DWORD/DINT 类按位计
                //（位数 = 载荷字节数 × 8），BYTE/REAL 等按字节计。
                let declared: u32 = match item.transport_size {
                    TS_BIT | TS_WORD | 0x05 | TS_DWORD | 0x07 => {
                        u32::from(payload.len() as u16) * 8
                    }
...
mod tests {
    ...
    fn read_var_returns_image_and_pads_to_even() {
        ...
        // 项 2（位）：FF 01 0008 + 载荷 01 + pad 00（BIT 的 length 域
        // 同样按位计但语义为「点数」——单点位声明 1；此处 mock 实现按
        // 载荷字节 ×8 与真机 WORD 语义对齐，golden 随实现固化）。
        assert_eq!(&data[6..10], &[RC_SUCCESS, TS_BIT, 0x00, 0x08]);
